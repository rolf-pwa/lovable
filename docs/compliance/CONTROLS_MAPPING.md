...
|---|---|---|---|
| Google (Workspace) | Full Gmail (read/send), Calendar, Drive, Docs, Sheets access | Staff productivity integration, Vault document storage, client correspondence | Access via OAuth (`google-auth`), tokens stored in `google_tokens` |
