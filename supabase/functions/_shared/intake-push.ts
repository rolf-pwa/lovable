// intake-push.ts
// Builds the CRM payload for a household and pushes it (HMAC-signed) to the
// Sovereignty Audit Agent so it can provision the Drive vault.
//
// Used by:
//   • crm-intake-push  — staff-triggered manual push / retry
//   • booking-enrollment — autonomous push right after a paid online booking
...
export interface IntakePushResult {
  ok: boolean;
  status: number;
  error?: string;
  itemsSent?: number;
  members?: number;
  response?: unknown;
  skipped?: "already_provisioned" | "already_pushed";
}

/**
 * True when this household already has (or is waiting on) an agent vault, so a
 * second automatic push would duplicate the Drive tree.
 */
export async function alreadyPushed(admin: any, householdId: string): Promise<boolean> {
  const { data: hh } = await admin
    .from("households")
    .select("intake_share_token, vault_root_folder_id")
    .eq("id", householdId)
    .maybeSingle();
  if (hh?.intake_share_token || hh?.vault_root_folder_id) return true;

  const { data: recent } = await admin
    .from("crm_intake_pushes")
    .select("id, status, created_at")
    .eq("household_id", householdId)
    .in("status", ["sent", "accepted", "provisioned"])
...
export async function pushHouseholdToIntakeAgent(
  admin: any,
  householdId: string,
  pushedBy: string | null,
): Promise<IntakePushResult> {
  ...
  const { data: household, error: hhErr } = await admin
    ...
    .eq("id", householdId)
    .single();
  if (hhErr || !household) return { ok: false, status: 404, error: "Household not found" };
  ...
  if (members.length === 0) {
    return { ok: false, status: 400, error: "Household has no members to push." };
  }
  ...
  console.log(`[intake-push] POST ${target} (household ${household.id})`);
  ...
  const fail = async (
    ...
    console.error(`[intake-push] ${message}`);
    if (logRow?.id) {
      const { error: upErr } = await admin
        ...
        .eq("id", logRow.id)
        .eq("status", "sent");
      if (upErr) console.error("[intake-push] log update failed", upErr.message);
    }
  ...
  // A redirect means we hit a website (e.g. a preview auth bridge), not the API.
  ...
  console.log(`[intake-push] agent responded ${res.status} (${contentType || "no content-type"})`);
